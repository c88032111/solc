...
# install for `gdtu` at the time of writing, so we unzip the ZIP file locally
# instead.  This will go away soon.
if [[ "$OSTYPE" == "darwin"* ]]; then
    GDTU_PATH="$REPO_ROOT/gdtu"
else
    GDTU_PATH="gdtu"
fi
...
while [ ! -S /tmp/test/ggdtu.ipc ]; do sleep 2; done

# And then run the Solidity unit-tests, pointing to that IPC endpoint.
"$REPO_ROOT"/build/test/soltest -- --ipcpath /tmp/test/ggdtu.ipc
ERROR_CODE=$?
pkill gdtu || true
sleep 4
pgrep gdtu && pkill -9 gdtu || true
exit $ERROR_CODE
