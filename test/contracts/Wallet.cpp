...
protected:
	...
	)
	{
		if (!s_compiledWallet)
		{
			...
			GDTU_TEST_REQUIRE_NO_THROW(m_compiler.compile(m_optimize, m_optimizeRuns), "Compiling contract failed");
...
BOOST_AUTO_TEST_CASE(add_owners)
{
	...
	sendGdtuer(account(1), 10 * gdtuer);
	m_sender = account(1);
	...
	m_sender = account(0);
	sendGdtuer(account(2), 10 * gdtuer);
	m_sender = account(2);
...
BOOST_AUTO_TEST_CASE(multisig_value_transfer)
{
	...
	BOOST_CHECK_EQUAL(balanceAt(Address(0x05)), 0);
	m_sender = account(0);
	sendGdtuer(account(3), 10 * gdtuer);
	m_sender = account(3);
	callContractFunction("confirm(bytes32)", ophash);
...
BOOST_AUTO_TEST_CASE(daylimit)
{
	...
	m_sender = account(0);
	sendGdtuer(account(4), 10 * gdtuer);
	m_sender = account(4);
	BOOST_REQUIRE(
