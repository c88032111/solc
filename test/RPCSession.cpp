...
*/
/** @file RPCSession.cpp
 * @author Dimtiry Khokhlov <dimitry@gdtudev.com>
 * @date 2016
 */
...
string RPCSession::gdtu_getCode(string const& _address, string const& _blockNumber)
{
	return rpcCall("gdtu_getCode", { quote(_address), quote(_blockNumber) }).asString();
}

RPCSession::TransactionReceipt RPCSession::gdtu_getTransactionReceipt(string const& _transactionHash)
{
	TransactionReceipt receipt;
	Json::Value const result = rpcCall("gdtu_getTransactionReceipt", { quote(_transactionHash) });
...
string RPCSession::gdtu_sendTransaction(TransactionData const& _td)
{
	return rpcCall("gdtu_sendTransaction", { _td.toJson() }).asString();
}

string RPCSession::gdtu_call(TransactionData const& _td, string const& _blockNumber)
{
	return rpcCall("gdtu_call", { _td.toJson(), quote(_blockNumber) }).asString();
}

string RPCSession::gdtu_sendTransaction(string const& _transaction)
{
	return rpcCall("gdtu_sendTransaction", { _transaction }).asString();
}

string RPCSession::gdtu_getBalance(string const& _address, string const& _blockNumber)
{
	string address = (_address.length() == 20) ? "0x" + _address : _address;
	return rpcCall("gdtu_getBalance", { quote(address), quote(_blockNumber) }).asString();
}

string RPCSession::gdtu_getStorageRoot(string const& _address, string const& _blockNumber)
{
	string address = (_address.length() == 20) ? "0x" + _address : _address;
	return rpcCall("gdtu_getStorageRoot", { quote(address), quote(_blockNumber) }).asString();
}
...
{
	u256 startBlock = fromBigEndian<u256>(fromHex(rpcCall("gdtu_blockNumber").asString()));
	...
	for (; polls < 14 && !mined; ++polls)
	{
		...
		if (fromBigEndian<u256>(fromHex(rpcCall("gdtu_blockNumber").asString())) >= startBlock + _number)
			mined = true;
		else
			sleepTime *= 2;
	}
