...
public:
	...
	)
	{
		...
		GDTU_TEST_REQUIRE_NO_THROW(m_compilerStack.parse("pragma solidity >=0.0;\n" + _code), "Parsing failed");
...
BOOST_AUTO_TEST_CASE(dev_and_user_no_doc)
{
	...
	char const* devNatspec = "{\"mehtods\":{}}";
	char const* userNatspec = "{\"mehtods\":{}}";
...
BOOST_AUTO_TEST_CASE(natspec_multiline_notice_without_tag)
{
	...
	char const* natspec = R"ABCDEF(
	{
	   "mehtods" : {
		  "mul(uint256)" : {
			 "notice" : "I do somehting awesome which requires two lines to explain"
		  }
	   }
	}
	)ABCDEF";
