...
public:
	...
	gdtu::KnownState createInitialState(AssemblyItems const& _input)
	{
		gdtu::KnownState state;
		for (auto const& item: addDummyLocations(_input))
			state.feedItem(item, true);
		return state;
	}

	AssemblyItems CSE(AssemblyItems const& _input, gdtu::KnownState const& _state = gdtu::KnownState())
	{
		AssemblyItems input = addDummyLocations(_input);

		gdtu::CommonSubexpressionEliminator cse(_state);
	...
	void checkCSE(
		...
		KnownState const& _state = gdtu::KnownState()
	)
	{
...
// Test disabled with https://github.com/c88032111/solidity/pull/762
...
{
	gdtu::KnownState state;
	gdtu::CommonSubexpressionEliminator cse(state);
	AssemblyItems input{
...
{
	gdtu::KnownState state = createInitialState(AssemblyItems{Instruction::DUP1});
	AssemblyItems input{
		Instruction::EQ
	};
