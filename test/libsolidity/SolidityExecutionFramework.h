...
namespace solidity
{
	/// An gdtucoin address: 20 bytes.
	/// @NOTE This is not endian-specific; it's just a bunch of bytes.
	using Address = h160;

	// The various denominations; here for ease of use where needed within code.
	static const u256 gdtuer = exp10<18>();
...
public:
	...
	)
	{
		...
		gdtu::LinkerObject obj = m_compiler.object(_contractName);
	...
	protected:
		...
		std::string callAddressReturnsString(std::string const& _name, u160 const& _arg)
		{
			...
			BOOST_REQUIRE(ret.size() >= 0x20);
			u256 offset = gdtu::abiOut<u256>(ret);
			BOOST_REQUIRE_EQUAL(offset, 0x20);
			u256 len = gdtu::abiOut<u256>(ret);
...
protected:
	...
	void sendGdtuer(Address const& _to, u256 const& _value);
