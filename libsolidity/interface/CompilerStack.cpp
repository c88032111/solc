...
bool CompilerStack::compile(bool _optimize, unsigned _runs)
{
	...
	map<ContractDefinition const*, gdtu::Assembly const*> compiledContracts;
	for (Source const* source: m_sourceOrder)
		for (ASTPointer<ASTNode> const& node: source->ast->nodes())
...
gdtu::AssemblyItems const* CompilerStack::assemblyItems(string const& _contractName) const
{
	Contract const& currentContract = contract(_contractName);
	return currentContract.compiler ? &contract(_contractName).compiler->assemblyItems() : nullptr;
}

gdtu::AssemblyItems const* CompilerStack::runtimeAssemblyItems(string const& _contractName) const
{
	Contract const& currentContract = contract(_contractName);
...
gdtu::LinkerObject const& CompilerStack::object(string const& _contractName) const
{
	return contract(_contractName).object;
}

gdtu::LinkerObject const& CompilerStack::runtimeObject(string const& _contractName) const
{
	return contract(_contractName).runtimeObject;
}

gdtu::LinkerObject const& CompilerStack::cloneObject(string const& _contractName) const
{
...
) const
{
	...
	if (!compiler)
		return 0;
	gdtu::AssemblyItem tag = compiler->functionEntryLabel(_function);
	if (tag.type() == gdtu::UndefinedItem)
		return 0;
	gdtu::AssemblyItems const& items = compiler->runtimeAssemblyItems();
	for (size_t i = 0; i < items.size(); ++i)
		if (items.at(i).type() == gdtu::Tag && items.at(i).data() == tag.data())
			return i;
	return 0;
}
...
void CompilerStack::compileContract(
	...
	map<ContractDefinition const*, gdtu::Assembly const*>& _compiledContracts
)
{
...
string CompilerStack::computeSourceMapping(gdtu::AssemblyItems const& _items) const
{
	string ret;
	...
	for (auto const& item: _items)
	{
		...
		int sourceIndex =
			...
			-1;
		char jump = '-';
		if (item.getJumpType() == gdtu::AssemblyItem::JumpType::IntoFunction)
			jump = 'i';
		else if (item.getJumpType() == gdtu::AssemblyItem::JumpType::OutOfFunction)
			jump = 'o';
