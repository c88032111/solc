...
{
	GeneratorState(ErrorList& _errors, gdtu::Assembly& _assembly):
	...
	gdtu::AssemblyItem const* findLabel(string const& _labelName) const
	{
		auto label = find_if(
			labels.begin(),
			labels.end(),
			[&](pair<string, gdtu::AssemblyItem> const& _label) { return _label.first == _labelName; }
		);
		return label != labels.end() ? &label->second : nullptr;
	}

	map<string, gdtu::AssemblyItem> labels;
	vector<pair<string, int>> variables; ///< name plus stack height
	ErrorList& errors;
	gdtu::Assembly& assembly;
};
...
public:
		...
		m_state(_state)
	{
		if (_identifierAccess)
			...
			m_identifierAccess = [](assembly::Identifier const&, gdtu::Assembly&, CodeGenerator::IdentifierContext) { return false; };
	}
	...
	void operator()(assembly::Identifier const& _identifier)
	{
		...
		else if (gdtu::AssemblyItem const* label = m_state.findLabel(_identifier.name))
...
gdtu::Assembly assembly::CodeGenerator::assemble(assembly::CodeGenerator::IdentifierAccess const& _identifierAccess)
{
	gdtu::Assembly assembly;
	GeneratorState state(m_errors, assembly);
	(LabelOrganizer(state))(m_parsedData);
...
void assembly::CodeGenerator::assemble(gdtu::Assembly& _assembly, assembly::CodeGenerator::IdentifierAccess const& _identifierAccess)
{
