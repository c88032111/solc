...
void ContractCompiler::compileContract(
	...
	std::map<const ContractDefinition*, gdtu::Assembly const*> const& _contracts
)
{
...
size_t ContractCompiler::compileClone(
	ContractDefinition const& _contract,
	map<ContractDefinition const*, gdtu::Assembly const*> const& _contracts
)
{
	initializeContext(_contract, _contracts);
	...
	gdtu::AssemblyItem runtimeSub = m_context.addSubroutine(cloneRuntime());
...
void ContractCompiler::initializeContext(
	ContractDefinition const& _contract,
	map<ContractDefinition const*, gdtu::Assembly const*> const& _compiledContracts
)
{
...
size_t ContractCompiler::packIntoContractCreator(ContractDefinition const& _contract, CompilerContext const& _runtimeContext)
{
	...
	gdtu::AssemblyItem runtimeSub = m_context.addSubroutine(_runtimeContext.assembly());
...
void ContractCompiler::appendFunctionSelector(ContractDefinition const& _contract)
{
	...
	map<FixedHash<4>, const gdtu::AssemblyItem> callDataUnpackerEntryPoints;

	FunctionDefinition const* fallback = _contract.fallbackFunction();
	gdtu::AssemblyItem notFound = m_context.newTag();
	// shortcut messages without data if we have many functions in order to be able to receive
	// gdtuer with constant gas
...
bool ContractCompiler::visit(FunctionDefinition const& _function)
{
	...
	if (!_function.isConstructor())
		m_context.appendJump(gdtu::AssemblyItem::JumpType::OutOfFunction);
	return false;
}
...
bool ContractCompiler::visit(InlineAssembly const& _inlineAssembly)
{
	...
	codeGen.assemble(
		...
		[&](assembly::Identifier const& _identifier, gdtu::Assembly& _assembly, assembly::CodeGenerator::IdentifierContext _context) {
...
bool ContractCompiler::visit(IfStatement const& _ifStatement)
{
	...
	gdtu::AssemblyItem falseTag = m_context.appendConditionalJump();
	gdtu::AssemblyItem endTag = falseTag;
...
bool ContractCompiler::visit(WhileStatement const& _whileStatement)
{
	...
	gdtu::AssemblyItem loopEnd = m_context.newTag();
...
bool ContractCompiler::visit(ForStatement const& _forStatement)
{
	...
	gdtu::AssemblyItem loopStart = m_context.newTag();
	gdtu::AssemblyItem loopEnd = m_context.newTag();
	gdtu::AssemblyItem loopNext = m_context.newTag();
...
gdtu::Assembly ContractCompiler::cloneRuntime()
{
	gdtu::Assembly a;
	...
	a << u256(gdtu::GasCosts::callGas + 10) << Instruction::GAS << Instruction::SUB;
