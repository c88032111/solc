...
gdtu::Assembly const& CompilerContext::compiledContract(const ContractDefinition& _contract) const
{
...
gdtu::AssemblyItem CompilerContext::functionEntryLabel(Declaration const& _declaration)
{
	return m_functionCompilationQueue.entryLabel(_declaration, *this);
}

gdtu::AssemblyItem CompilerContext::functionEntryLabelIfExists(Declaration const& _declaration) const
{
	return m_functionCompilationQueue.entryLabelIfExists(_declaration);
}

gdtu::AssemblyItem CompilerContext::virtualFunctionEntryLabel(FunctionDefinition const& _function)
{
...
gdtu::AssemblyItem CompilerContext::superFunctionEntryLabel(FunctionDefinition const& _function, ContractDefinition const& _base)
{
	solAssert(!m_inheritanceHierarchy.empty(), "No inheritance hierarchy set.");
...
CompilerContext& CompilerContext::appendJump(gdtu::AssemblyItem::JumpType _jumpType)
{
	gdtu::AssemblyItem item(Instruction::JUMP);
...
{
	gdtu::Assembly& sub = m_asm.sub(_subIndex);
	sub.injectStart(Instruction::POP);
	sub.injectStart(fromBigEndian<u256>(binaryVersion()));
}

gdtu::AssemblyItem CompilerContext::virtualFunctionEntryLabel(
...
gdtu::AssemblyItem CompilerContext::FunctionCompilationQueue::entryLabel(
...
)
{
	...
	{
		gdtu::AssemblyItem tag(_context.newTag());
...
gdtu::AssemblyItem CompilerContext::FunctionCompilationQueue::entryLabelIfExists(Declaration const& _declaration) const
{
	auto res = m_entryLabels.find(&_declaration);
	return res == m_entryLabels.end() ? gdtu::AssemblyItem(gdtu::UndefinedItem) : res->second.tag();
}
