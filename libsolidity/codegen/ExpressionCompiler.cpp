...
bool ExpressionCompiler::visit(Conditional const& _condition)
{
	...
	gdtu::AssemblyItem trueTag = m_context.appendConditionalJump();
	...
	gdtu::AssemblyItem endTag = m_context.appendJumpToNew();
	m_context << trueTag;
...
bool ExpressionCompiler::visit(FunctionCall const& _functionCall)
{
	...
	else
	{
		...
		case Location::Internal:
		{
			...
			gdtu::AssemblyItem returnLabel = m_context.pushNewTag();
			for (unsigned i = 0; i < arguments.size(); ++i)
			{
				arguments[i]->accept(*this);
			...
			m_context.appendJump(gdtu::AssemblyItem::JumpType::IntoFunction);
		...
		case Location::Creation:
		{
			...
			gdtu::Assembly const& assembly = m_context.compiledContract(contract);
			utils().fetchFreeMemoryPointer();
			// pushes size
			gdtu::AssemblyItem subroutine = m_context.addSubroutine(assembly);
		...
		case Location::Send:
			...
			m_context << u256(gdtu::GasCosts::callStipend);
			arguments.front()->accept(*this);
			utils().convertType(
...
void ExpressionCompiler::appendAndOrOperatorCode(BinaryOperation const& _binaryOperation)
{
	...
	if (c_op == Token::And)
		m_context << Instruction::ISZERO;
	gdtu::AssemblyItem endLabel = m_context.appendConditionalJump();
	m_context << Instruction::POP;
...
)
{
	...
	else
	{
		...
		u256 gasNeededByCaller = gdtu::GasCosts::callGas + 10;
		if (_functionType.valueSet())
			gasNeededByCaller += gdtu::GasCosts::callValueTransferGas;
		if (!isCallCode && !isDelegateCall && !existenceChecked)
			gasNeededByCaller += gdtu::GasCosts::callNewAccountGas; // we never know
