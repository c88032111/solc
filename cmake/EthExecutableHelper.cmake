...
macro(gdtu_add_executable EXECUTABLE)
	set (extra_macro_args ${ARGN})
	set (options)
	set (one_value_args ICON)
	set (multi_value_args UI_RESOURCES WIN_RESOURCES)
	cmake_parse_arguments (GDTU_ADD_EXECUTABLE "${options}" "${one_value_args}" "${multi_value_args}" "${extra_macro_args}")

	if (APPLE)

		add_executable(${EXECUTABLE} MACOSX_BUNDLE ${SRC_LIST} ${HEADERS} ${GDTU_ADD_EXECUTABLE_UI_RESOURCES})
		set(PROJECT_VERSION "${GDTU_VERSION}")
		...
		set(MACOSX_BUNDLE_ICON_FILE ${GDTU_ADD_EXECUTABLE_ICON})
		set_target_properties(${EXECUTABLE} PROPERTIES MACOSX_BUNDLE_INFO_PLIST "${CMAKE_CURRENT_SOURCE_DIR}/gdtucoinMacOSXBundleInfo.plist.in")
		...
		add_executable(${EXECUTABLE} ${GDTU_ADD_EXECUTABLE_UI_RESOURCES}  ${GDTU_ADD_EXECUTABLE_WIN_RESOURCES} ${SRC_LIST} ${HEADERS})
	endif()

endmacro()

macro(gdtu_simple_add_executable EXECUTABLE)
...
macro(gdtu_copy_dll EXECUTABLE DLL)
	...
	if(DEFINED MSVC)
		...
		add_custom_command(TARGET ${EXECUTABLE}
			...
			-P "${GDTU_SCRIPTS_DIR}/copydlls.cmake"
		)
	endif()
endmacro()

macro(gdtu_copy_dlls EXECUTABLE)
	foreach(dll ${ARGN})
		gdtu_copy_dll(${EXECUTABLE} ${dll})
	endforeach(dll)
endmacro()


macro(gdtu_install_executable EXECUTABLE)
