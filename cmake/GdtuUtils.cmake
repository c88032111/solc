...
macro(gdtu_add_test NAME)
	...
	while (index LESS count)
		...
		add_test(NAME "${NAME}.${index}" COMMAND testgdtu ${run_test} ${test_arguments})
	...
	add_custom_target("test.${NAME}"
		...
		COMMAND ${CMAKE_COMMAND} -DGDTU_TEST_NAME="${NAME}" -DCTEST_COMMAND="${CTEST_COMMAND}" -P "${GDTU_SCRIPTS_DIR}/runtest.cmake"
	)

endmacro()

# Creates C resources file from files
function(gdtu_add_resources RESOURCE_FILE OUT_FILE GDTU_RES_DIR)
	include("${RESOURCE_FILE}")
	set(OUTPUT  "${GDTU_RESOURCE_LOCATION}/${GDTU_RESOURCE_NAME}.hpp")
	#message(FATAL_ERROR "res:! ${GDTU_RESOURCE_LOCATION}")
	include_directories("${GDTU_RESOURCE_LOCATION}")
	set(${OUT_FILE} "${OUTPUT}"  PARENT_SCOPE)

	set(filenames "${RESOURCE_FILE}")
	list(APPEND filenames "${GDTU_SCRIPTS_DIR}/resources.cmake")
	foreach(resource ${GDTU_RESOURCES})
		list(APPEND filenames "${${resource}}")
	endforeach(resource)

	add_custom_command(OUTPUT ${OUTPUT}
		COMMAND ${CMAKE_COMMAND} -DGDTU_RES_FILE="${RESOURCE_FILE}" -DGDTU_RES_DIR="${GDTU_RES_DIR}"  -P "${GDTU_SCRIPTS_DIR}/resources.cmake"
		DEPENDS ${filenames}
	)
endfunction()
