...
we would not be able to send Gdtuer to it in the call ``feed.info.value(10).gas(800)()``.
...
As seen in the example, it is possible to forward Gdtuer to the creation,
...
In the following example, we show how ``throw`` can be used to easily revert an Gdtuer transfer and also how to check the return value of ``send``::
...
3. If a non-existent function on a library is called or Gdtuer is sent to a library.
...
This document does not want to be a full description of the gdtucoin virtual machine, but the
...
+-------------------------+------+-----------------------------------------------------------------+
| callvalue               |      | wei sent toggdtuer with the current call                         |
+-------------------------+------+-----------------------------------------------------------------+
