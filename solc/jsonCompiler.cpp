...
Json::Value estimateGas(CompilerStack const& _compiler, string const& _contract)
{
	...
	if (!_compiler.assemblyItems(_contract) && !_compiler.runtimeAssemblyItems(_contract))
		return gasEstimates;
	if (gdtu::AssemblyItems const* items = _compiler.assemblyItems(_contract))
	{
		...
		creationGas[1] = gasToJson(bytecodeSize * gdtu::GasCosts::createDataGas);
		gasEstimates["creation"] = creationGas;
	}
	if (gdtu::AssemblyItems const* items = _compiler.runtimeAssemblyItems(_contract))
	{
